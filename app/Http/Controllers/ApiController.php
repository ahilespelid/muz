...
public function stoimost(Request $request){
    $validator = Validator::make($data = json_decode($request->json, true), ['times' => 'required|array|min:1']);
...
public function listBases(){
    ...
    curl_setopt_array($this->curl, [
        CURLOPT_URL => $url = $this->url.'/api/bases/names', 
        CURLOPT_RETURNTRANSFER => true, CURLOPT_POST => true, 
        CURLOPT_POSTFIELDS => http_build_query(['token' => $this->token])
        ]); pa($url.'?'.http_build_query(['token' => $this->token]));
return (is_json($json = curl_exec($this->curl))) ? json_decode($json, true) : $json;}

public function listOrders(int $agoMonth = 1){
    if(empty($this->token)){return null;}
    $date = date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s') . ' -'.$agoMonth.' month'));
    curl_setopt_array($this->curl, [
        CURLOPT_URL => $this->url.'/api/orders/sync-list', 
        CURLOPT_RETURNTRANSFER => true, CURLOPT_POST => true, 
        CURLOPT_POSTFIELDS => http_build_query(['token' => $this->token, 'date' => $date])
        ]);

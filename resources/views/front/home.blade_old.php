    ...
    <div id="head" style="width:100%; height: 50px; background: #ffffff; min-height: 55px;">
        <div class="head">
            <div class="head_el">
                ...
                <a href="{{ route('front.admin', ['baseId' => $curCorpuses['muzid'], 'dateIn' => date('Y-m-d')]) }}"><div class="head_el__button_today">Админка</div></a>
...
@foreach($rooms ?? [] as $claseid => $n)
                ...
                @php $k = key(array_filter($Classes, function($ar) use($claseid){return ($claseid == $ar['id']);})); @endphp
                @if(!empty($Classes[$k]))
